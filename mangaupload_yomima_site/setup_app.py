...
_platform_log_file = None

def read_process_output(process, stream, prefix=""):
    """プロセスの出力をリアルタイムで読み取り、ファイルに書き出す"""
    global _platform_log_file
    if _platform_log_file is None:
        _platform_log_file = open("/content/platform_debug.log", "w", buffering=1)
    while True:
        line = stream.readline()
        if line:
            _platform_log_file.write(f"{prefix}{line.strip()}\n")
            _platform_log_file.flush()
        elif process.poll() is not None:
            try:
                remaining = stream.read()
                if remaining:
                    _platform_log_file.write(f"{prefix}{remaining.strip()}\n")
                    _platform_log_file.flush()
            except ValueError:
                pass
            break
...
def setup_cloudflare_tunnel():
    ...
    if not wait_for_flask_server(port=8000, timeout=60):
        print("❌ FastAPI起動失敗")
        return None, None, ""
...
##    # Cloudflareトンネルの起動
##    print("🌐 Cloudflare トンネルを開始しています...")
##    tunnel_process = subprocess.Popen(['/usr/local/bin/cloudflared', 'tunnel', '--url', 'http://localhost:8000'],
##                                     stdout=subprocess.PIPE,
##                                     stderr=subprocess.PIPE,
##                                     text=True,
##                                     bufsize=1)
...
##        # ★ ビューワーが読み取れるようにURLをファイルに書き出す
##        with open(f"{current_directory}/platform_url.txt", "w") as f:
##            f.write(url)
##        print(f"📝 プラットフォームURLを platform_url.txt に保存しました")
##    else:
...
def wait_for_flask_server(port=8000, timeout=60):
